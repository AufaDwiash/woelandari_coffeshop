...
if (isset($_POST['simpan_gallery']) || isset($_POST['update_gallery'])) {
    ...
    if (isset($_POST['simpan_gallery'])) {
        ...
        mysqli_query($conn, $update) ? redirectWithMsg("gallery_crud.php?tab=gallery&page=$page" . ($search ? "&search=" . urlencode($search) : ""), " Gallery berhasil diperbarui", false) : redirectWithMsg("gallery_crud.php?tab=gallery&page=$page", "❌ Gagal mengupdate gallery", true);
    }
}
...
if (isset($_GET['hapus_gallery'])) {
    ...
    redirectWithMsg("gallery_crud.php?tab=gallery&page=$page" . ($search ? "&search=" . urlencode($search) : ""), " Gallery berhasil dihapus", false);
}
...
if (isset($_POST['simpan_event']) || isset($_POST['update_event'])) {
    ...
    if (isset($_POST['simpan_event'])) {
        ...
        mysqli_query($conn, $update) ? redirectWithMsg("gallery_crud.php?tab=event&page=$page" . ($search ? "&search=" . urlencode($search) : ""), " Event berhasil diperbarui", false) : redirectWithMsg("gallery_crud.php?tab=event&page=$page", "❌ Gagal mengupdate event", true);
    }
}
...
if (isset($_GET['hapus_event'])) {
    ...
    redirectWithMsg("gallery_crud.php?tab=event&page=$page" . ($search ? "&search=" . urlencode($search) : ""), " Event berhasil dihapus", false);
}
...
function renderGalleryTable($conn, $search, $page, $limit, $offset)
{
        ...
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th class="col-title">JUDUL & DESKRIPSI</th>
                    <th class="col-status">KATEGORI</th>
            ...
            <tbody>
                <?php if (mysqli_num_rows($q) > 0):
                    while ($r = mysqli_fetch_assoc($q)): ?>
                        <tr>
                            <td style="text-align:center"><?php if ($r['file_foto']): ?><img
                                        src="data:image/jpeg;base64,<?= base64_encode($r['file_foto']) ?>"
                                        class="thumb-img"><?php else: ?>
                                    <div class="no-img"></div><?php endif; ?>
                            </td>
                            <td><strong><?= htmlspecialchars($r['judul']) ?></strong><br><small><?= htmlspecialchars(substr($r['deskripsi'], 0, 60)) ?>...</small>
                            </td>
                            <td style="text-align:center"><span
                                    class="status-badge <?= $r['id_event'] ? 'status-active' : 'status-pending' ?>"><?= $r['judul_event'] ? strtoupper(htmlspecialchars($r['judul_event'])) : 'FOTO BIASA' ?></span>
                            </td>
                            <td>
                                <div class="action-buttons"><a
                                        href="?tab=gallery&edit_gallery=<?= $r['id_gallery'] ?>&search=<?= urlencode($search) ?>&page=<?= $page ?>"
                                        class="btn-action btn-edit-action"><i class="fas fa-pencil-alt"></i> EDIT</a>
                                    <button type="button" class="btn-action btn-delete-action delete-btn"
                                        data-id="<?= $r['id_gallery'] ?>" data-name="<?= htmlspecialchars($r['judul']) ?>"
                                        data-type="gallery" data-search="<?= htmlspecialchars($search) ?>"
                                        data-page="<?= $page ?>"><i class="fas fa-trash-alt"></i> HAPUS</button>
                                </div>
                            </td>
                        </tr>
                    <?php endwhile;
                    ...
                    <tr>
                        <td colspan="4" style="text-align:center; padding:40px;"><i class="fas fa-image"></i> [ DATA GALLERY
                            KOSONG ]</td>
                    </tr>
                <?php endif; ?>
            </tbody>
    ...
    <?php if ($totalPages > 1): ?>
        <div class="pagination-area"><button class="btn btn-secondary pagi-prev" data-page="<?= $page - 1 ?>" <?= $page <= 1 ? 'disabled' : '' ?>><i class="fas fa-chevron-left"></i> PREV</button><span>HALAMAN <?= $page ?> DARI
                <?= $totalPages ?></span><button class="btn btn-secondary pagi-next" data-page="<?= $page + 1 ?>" <?= $page >= $totalPages ? 'disabled' : '' ?>>NEXT <i class="fas fa-chevron-right"></i></button></div>
    <?php endif;
    return ob_get_clean();
}
        ...
        <table class="data-table">
            ...
            <tbody>
                <?php if (mysqli_num_rows($q) > 0):
                    while ($r = mysqli_fetch_assoc($q)): ?>
                        <tr>
                            <td style="text-align:center"><?php if ($r['foto_cover']): ?><img
                                        src="data:image/jpeg;base64,<?= base64_encode($r['foto_cover']) ?>"
                                        class="thumb-img"><?php else: ?>
                                    <div class="no-img"></div><?php endif; ?>
                            </td>
                            <td><strong><?= htmlspecialchars($r['judul_event']) ?></strong> <?php if ($r['is_featured']): ?><span
                                        class="featured-star"><i class="fas fa-star"></i></span><?php endif; ?></td>
                            <td><i class="far fa-calendar-alt"></i> <?= date('d M Y', strtotime($r['tanggal_event'])) ?></td>
                            <td style="text-align:center; vertical-align:middle;">
                                <span
                                    class="status-badge <?= $r['status_event'] == 'selesai' ? 'status-pending' : 'status-active' ?>"><?= strtoupper($r['status_event']) ?></span>
                            </td>
                            <td>
                                <div class="action-buttons"><a
                                        href="?tab=event&edit_event=<?= $r['id_event'] ?>&search=<?= urlencode($search) ?>&page=<?= $page ?>"
                                        class="btn-action btn-edit-action"><i class="fas fa-pencil-alt"></i> EDIT</a>
                                    <button type="button" class="btn-action btn-delete-action delete-btn"
                                        data-id="<?= $r['id_event'] ?>" data-name="<?= htmlspecialchars($r['judul_event']) ?>"
                                        data-type="event" data-search="<?= htmlspecialchars($search) ?>" data-page="<?= $page ?>"><i
                    ...
                    <tr>
                        <td colspan="5" style="text-align:center; padding:40px;"><i class="fas fa-calendar-alt"></i> [ DATA
                            EVENT KOSONG ]</td>
                    </tr>
                <?php endif; ?>
            </tbody>
...
if (isset($_GET['ajax'])) {
    if ($active_tab == 'gallery')
        echo renderGalleryTable($conn, $search, $page, $limit, $offset);
    else
        echo renderEventTable($conn, $search, $page, $limit, $offset);
    exit;
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <main class="main-wrapper">
        <div class="mobile-header">
            <div><i class="fas fa-images" style="color: var(--red);"></i> WOELANDARI</div><button id="hamburgerBtn"
                style="background:none; border:none; font-size:1.6rem;"><i class="fas fa-bars"></i></button>
        </div>
        <section class="paper">
            <div class="tape"></div>
            <div class="spec-header"><span><i class="fas fa-folder-open"></i> Kelola Gallery & Events</span><span>DATE:
                    <?= date('d/m/Y') ?></span></div>
            <?php if ($msg_display): ?>
                <div class="alert-msg"><i class="fas fa-check-circle"></i> <?= $msg_display ?></div><?php endif; ?>
            <?php if ($error_display): ?>
                <div class="alert-msg error"><i class="fas fa-exclamation-circle"></i> <?= $error_display ?></div>
            <?php endif; ?>

            <div class="tab-buttons">
                <a href="?tab=gallery&page=1<?= $search ? '&search=' . urlencode($search) : '' ?>"
                    class="tab-btn <?= $active_tab == 'gallery' ? 'active' : '' ?>"><i class="fas fa-image"></i>
                    GALLERY</a>
                <a href="?tab=event&page=1<?= $search ? '&search=' . urlencode($search) : '' ?>"
                    class="tab-btn <?= $active_tab == 'event' ? 'active' : '' ?>"><i class="fas fa-calendar-alt"></i>
                    EVENTS</a>
            </div>

            <div class="search-area">
                <div class="search-wrapper"><i class="fas fa-search"></i><input type="text" id="searchInput"
                        class="search-input"
                        placeholder="Cari judul <?= $active_tab == 'gallery' ? 'foto' : 'event' ?>..."
                        value="<?= htmlspecialchars($search) ?>"></div>
                <button class="btn btn-primary" id="searchBtn">CARI DATA</button>
                <?php if ($search): ?><a href="gallery_crud.php?tab=<?= $active_tab ?>&page=1"
                        class="btn btn-secondary">RESET</a><?php endif; ?>
                <button class="btn btn-primary add-entry-btn"
                    style="background: var(--red); box-shadow: 4px 4px 0 var(--navy);"><i class="fas fa-plus"></i> ADD
                    ENTRY</button>
            </div>
    ...
    <div class="modal" id="modalGallery">
        <div class="modal-content">
            ...
            <div class="modal-header-area">
                <div class="spec-header" style="margin-bottom:10px;"><span id="galleryModalTitle">TAMBAH FOTO
                        GALLERY</span></div>
            </div>
            <div class="modal-body-scroll">
                <form id="formGallery"
                    action="gallery_crud.php?tab=gallery&page=<?= $page ?><?= $search ? '&search=' . urlencode($search) : '' ?>"
                    ...
                    <div class="form-group"><label class="form-label">JUDUL FOTO</label><input type="text" name="judul"
                            id="gal_judul" class="form-input" value="<?= htmlspecialchars($gal_judul) ?>" required>
                    </div>
                    <div class="form-group"><label class="form-label">DESKRIPSI SINGKAT</label><textarea
                            name="deskripsi" id="gal_deskripsi" class="form-input"
                            rows="2"><?= htmlspecialchars($gal_deskripsi) ?></textarea></div>
                    <div class="form-group">
                        <label class="form-label">TIPE KONTEN</label>
                        <select name="tipe_koneksi" id="tipe_koneksi" class="form-select">
                            <option value="biasa" <?= (!$gal_event || $gal_event == 'NULL') ? 'selected' : '' ?>>Foto
                                Gallery Biasa</option>
                            <option value="event" <?= ($gal_event && $gal_event != 'NULL') ? 'selected' : '' ?>>Terhubung
                                dengan Event</option>
                        </select>
                        <div id="eventSelectWrapper"
                            style="margin-top:10px; <?= ($gal_event && $gal_event != 'NULL') ? '' : 'display:none;' ?>">
                    ...
                    <div class="upload-box-safe">
                        ...
                        <?php if ($gal_foto): ?>
                            <div style="margin-bottom:10px;"><img
                                    src="data:image/jpeg;base64,<?= base64_encode($gal_foto) ?>"
                                    style="width:80px; border:2px solid var(--navy);">
                        ...
                        <div id="previewAreaGallery" style="display:none; margin-top:15px;"><img id="preview_gallery"
                                style="max-width:100px; border:2px solid var(--navy);">
                            <p style="color:green;">✓ CROP SELESAI</p>
                        </div>
                    </div>
                    <div style="display:flex; justify-content:flex-end; gap:15px; margin-top:25px;"><button
                            type="button" class="btn btn-secondary close-modal-btn">BATAL</button><button type="submit"
                            name="<?= $edit_gal_mode ? 'update_gallery' : 'simpan_gallery' ?>"
                            class="btn btn-primary">SIMPAN DATA</button></div>
                </form>
            </div>
        </div>
    </div>
    ...
    <div class="modal" id="modalEvent">
        <div class="modal-content">
            <div class="tape" style="top:-16px; width:100px; height:25px;"></div>
            <div class="modal-header-area">
                <div class="spec-header" style="margin-bottom:10px;"><span
                        id="eventModalTitle"><?= $edit_event_mode ? 'UPDATE EVENT' : 'BUAT EVENT BARU' ?></span></div>
            </div>
            <div class="modal-body-scroll">
                <form id="formEvent"
                    action="gallery_crud.php?tab=event&page=<?= $page ?><?= $search ? '&search=' . urlencode($search) : '' ?>"
                    method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group"><label class="form-label">JUDUL EVENT</label><input type="text"
                            name="judul_event" id="ev_judul" class="form-input"
                            value="<?= htmlspecialchars($ev_judul) ?>" required></div>
                    <div class="form-grid two-cols">
                        <div class="form-group"><label class="form-label">TANGGAL PELAKSANAAN</label><input type="date"
                                name="tanggal_event" id="ev_tanggal" class="form-input" value="<?= $ev_tanggal ?>"
                                required></div>
                        <div class="form-group"><label class="form-label">STATUS EVENT</label>
                            <select name="status_event" id="ev_status" class="form-select">
                                <option value="mendatang" <?= ($ev_status == 'mendatang') ? 'selected' : '' ?>>MENDATANG
                                </option>
                                <option value="selesai" <?= ($ev_status == 'selesai') ? 'selected' : '' ?>>SELESAI</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group"><label class="form-label">DESKRIPSI LENGKAP</label><textarea
                            name="deskripsi_event" id="ev_deskripsi" class="form-input" rows="3"
                            required><?= htmlspecialchars($ev_deskripsi) ?></textarea></div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="checkbox-group">
                            ...
                            <?php if ($is_checkbox_disabled): ?>
                                <p style="font-size:0.7rem; color:var(--red); margin-top:5px;"><i
                                        class="fas fa-info-circle"></i> Event sudah selesai, tidak bisa disematkan.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="upload-box-safe">
                        ...
                        <?php if ($ev_foto): ?>
                            <div style="margin-bottom:10px;"><img
                                    src="data:image/jpeg;base64,<?= base64_encode($ev_foto) ?>"
                                    style="width:80px; border:2px solid var(--navy);">
                                <p style="font-size:0.75rem;">[ COVER SAAT INI ]</p>
                            </div>
                        <?php endif; ?>
                        <input type="file" id="foto_cover_event" name="foto_cover" accept="image/*" <?= $edit_event_mode ? '' : 'required' ?>>
                        <div id="previewAreaEvent" style="display:none; margin-top:15px;"><img id="preview_event"
                                style="max-width:100px; border:2px solid var(--navy);">
                            <p style="color:green;">✓ CROP SELESAI</p>
                        </div>
                    </div>
                    <div style="display:flex; justify-content:flex-end; gap:15px; margin-top:25px;"><button
                            type="button" class="btn btn-secondary close-modal-btn">BATAL</button><button type="submit"
                            name="<?= $edit_event_mode ? 'update_event' : 'simpan_event' ?>"
                            class="btn btn-primary">SIMPAN DATA</button></div>
                </form>
            </div>
        </div>
    ...
    <div id="cropModal" class="modal" style="z-index:2100;">
        <div class="modal-content" style="max-width:460px; padding:20px;">
            ...
            <div style="border:2px solid var(--navy); background:#000;"><img id="cropImage" src=""
                    style="max-width:100%;"></div>
            <div style="display:flex; justify-content:flex-end; gap:15px; margin-top:20px;"><button
                    class="btn btn-secondary" id="cancelCropBtn">BATAL</button><button class="btn btn-primary"
                    id="cropConfirmBtn" style="background:var(--red);">CROP & SET</button></div>
        </div>
    </div>
    ...
    <div id="deleteConfirmModal" class="confirm-modal">
        <div class="confirm-modal-content">
            ...
            <div class="confirm-modal-body">
                ...
                <p style="font-size:0.8rem;"><i class="fas fa-info-circle"></i> Data dihapus tidak dapat dikembalikan!
                </p>
            </div>
            <div class="confirm-modal-footer"><button class="btn btn-secondary" id="cancelDeleteBtn">BATAL</button><a
                    href="#" id="confirmDeleteBtn" class="btn btn-danger">HAPUS</a></div>
        </div>
    </div>
    ...
    <script>
        ...
        container.addEventListener('click', e => {
            ...
            if (pagi && !pagi.disabled) {
                e.preventDefault();
                loadTable(pagi.dataset.page, document.getElementById('searchInput').value);
                return;
            }
        ...
        function resetAndOpenModal(modalId) {
            ...
            modal.querySelectorAll('input[type="hidden"]').forEach(h => {
                if (h.id.includes('cropped')) h.value = '';
            });
        ...
        <?php if ($edit_gal_mode): ?>resetAndOpenModal('modalGallery');
            document.getElementById('gal_id').value = '<?= $gal_id ?>';
            document.getElementById('gal_judul').value = '<?= addslashes($gal_judul) ?>';
            document.getElementById('gal_deskripsi').value = '<?= addslashes($gal_deskripsi) ?>';
            if (<?= $gal_event ? 'true' : 'false' ?>) {
                document.getElementById('tipe_koneksi').value = 'event';
                document.getElementById('eventSelectWrapper').style.display = 'block';
                document.getElementById('gal_id_event').value = '<?= $gal_event ?>';
            }
        <?php endif; ?>
        <?php if ($edit_event_mode): ?>resetAndOpenModal('modalEvent');
            document.getElementById('ev_id').value = '<?= $ev_id ?>';
            document.getElementById('ev_judul').value = '<?= addslashes($ev_judul) ?>';
            document.getElementById('ev_tanggal').value = '<?= $ev_tanggal ?>';
            document.getElementById('ev_status').value = '<?= $ev_status ?>';
            document.getElementById('ev_deskripsi').value = '<?= addslashes($ev_deskripsi) ?>';
            <?php if (!$ev_is_expired && $ev_featured): ?>document.querySelector('input[name="is_featured"]').checked = true;
            <?php endif; ?> <?php endif; ?>
        ...
        const cropModal = document.getElementById('cropModal'),
            ...
            cropConfirm = document.getElementById('cropConfirmBtn'),
        ...
        // ========== OTOMATIS STATUS EVENT BERDASARKAN TANGGAL ==========
        function updateStatusByDate() {
            const dateInput = document.getElementById('ev_tanggal');
            const statusSelect = document.getElementById('ev_status');
            if (!dateInput || !statusSelect) return;

            const selectedDate = dateInput.value;
            if (!selectedDate) return; // kosong, biarkan default

            const today = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
            if (selectedDate < today) {
                statusSelect.value = 'selesai';
            } else {
                statusSelect.value = 'mendatang';
            }
        }

        // Pasang event listener pada field tanggal di modal event
        const evTanggalInput = document.getElementById('ev_tanggal');
        if (evTanggalInput) {
            evTanggalInput.addEventListener('change', updateStatusByDate);
        }

        // Override fungsi resetAndOpenModal agar setelah mengisi nilai edit, status juga disesuaikan
        const originalResetAndOpenModal = resetAndOpenModal;
        window.resetAndOpenModal = function (modalId) {
            originalResetAndOpenModal(modalId);
            if (modalId === 'modalEvent') {
                // Tunggu sebentar agar nilai tanggal sudah terisi (untuk mode edit)
                setTimeout(() => {
                    updateStatusByDate();
                }, 50);
            }
        };
        resetAndOpenModal = window.resetAndOpenModal;
        document.getElementById('file_foto_gallery')?.addEventListener('change', e => {
            if (e.target.files[0]) initCropper(e.target.files[0], 'gallery');
        });
        document.getElementById('foto_cover_event')?.addEventListener('change', e => {
        ...
        document.getElementById('deleteConfirmModal')?.addEventListener('click', e => {
            if (e.target === document.getElementById('deleteConfirmModal')) e.target.style.display = 'none';
        });
        window.addEventListener('popstate', e => {
            if (e.state) {
                currentTab = e.state.tab;
                loadTable(e.state.page, e.state.search, true);
